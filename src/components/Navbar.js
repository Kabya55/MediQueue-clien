...
import { Sun, Moon, LogOut, User, BookOpen, GraduationCap, Menu } from 'lucide-react';
...
export default function Navbar() {
  ...
  const mobileLinkClass = (path) =>
    `font-medium ${isActive(path) ? 'text-primary bg-primary/10 font-semibold' : 'text-base-content/85'}`;

  return (
    <div className="navbar sticky top-0 z-50 glass-nav border-b border-base-200 px-4 sm:px-8 shadow-sm">
      <div className="navbar-start">
        <div className="dropdown lg:hidden">
          <label tabIndex={0} className="btn btn-ghost btn-circle text-base-content" aria-label="Open menu">
          ...
          >
            ...
            {user && (
              <>
                <li>
                  <Link href="/add-tutor" className={mobileLinkClass('/add-tutor')}>
                    Add Tutor
                  </Link>
                </li>
                <li>
                  <Link href="/my-tutors" className={mobileLinkClass('/my-tutors')}>
                    My Tutors
                  </Link>
                </li>
                <li>
                  <Link href="/my-booked-sessions" className={mobileLinkClass('/my-booked-sessions')}>
                    My Bookings
                  </Link>
                </li>
              </>
      ...
      <div className="navbar-end gap-3">
        <button
          ...
          title={theme === 'emerald' ? 'Switch to Dark Mode' : 'Switch to Light Mode'}
          aria-label="Toggle theme"
        >
          {theme === 'emerald' ? <Moon className="h-5 w-5" /> : <Sun className="h-5 w-5" />}
        </button>

        {user ? (
          <div className="dropdown dropdown-end">
            <label tabIndex={0} className="btn btn-ghost btn-circle avatar online ring-2 ring-primary/20 ring-offset-2">
              <div className="w-9 h-9 rounded-full overflow-hidden">
                <img
                  src={user.image || user.photoUrl || 'https://via.placeholder.com/150'}
                  alt={user.name || 'User Profile'}
                  onError={(e) => {
                    e.target.src = 'https://api.dicebear.com/7.x/adventurer/svg?seed=' + encodeURIComponent(user.name || 'avatar');
                  }}
                />
              </div>
            </label>
            ...
            >
              <div className="px-3 py-2 border-b border-base-200 mb-1">
                <p className="font-semibold text-base text-base-content truncate">{user.name}</p>
                <p className="text-xs text-base-content/60 truncate">{user.email}</p>
              </div>
              <li>
                <Link href="/my-tutors" className="flex items-center gap-2 text-base-content py-2">
                  <User className="h-4 w-4 text-base-content/60" /> My Profile & Tutors
                </Link>
              </li>
              <li>
                <Link href="/my-booked-sessions" className="flex items-center gap-2 text-base-content py-2">
                  <BookOpen className="h-4 w-4 text-base-content/60" /> My Bookings
                </Link>
              </li>
          ...
          <div className="flex items-center gap-2">
            <Link href="/login" className="btn btn-sm rounded-full font-semibold text-base-content/90 hover:text-primary bg-base-200/40 hover:bg-base-200/60 border border-base-200/50 backdrop-blur-md transition-all shadow-sm">
              Login
            </Link>
            <Link href="/register" className="btn btn-sm rounded-full px-5 shadow-md font-semibold bg-primary/90 hover:bg-primary text-primary-content border border-primary/30 backdrop-blur-md transition-all hover:shadow-lg">
              Register
            </Link>
          </div>
        )}
